...
void Spec_Write_Scalar( CCTK_ARGUMENTS, CCTK_REAL **scalar );
void Spec_Analyze_Scalar( CCTK_ARGUMENTS );
...
void Spec_Scalar( CCTK_ARGUMENTS, CCTK_REAL **scalar )
{
    ...
    for(k=0; k < kend; k++)
	{
		for(j=0; j < jend; j++)
		{
			for(i=0; i < iend; i++)
			{
                ...
                temp_phi0[ index ] = scalar[0][index];
                temp_phi1[ index ] = 0.0;
                temp_phi2[ index ] = 0.0;
                temp_phi3[ index ] = 0.0;
	...
	Spec_Analyze_Scalar( CCTK_PASS_CTOC );
	Spec_Write_Scalar( CCTK_PASS_CTOC, scalar );
}
...
void Spec_Analyze_Scalar( CCTK_ARGUMENTS )
{
    ...
    for(k = 1; k < local_n0; k++) 
    {
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
				temp_phi1[i + j*sizex + k*sizex*sizey] = creal(phi_out[i + j*sizex + k*sizex*sizey]/(sizex*sizey*sizez));
				temp_phi2[i + j*sizex + k*sizex*sizey] = cimag(phi_out[i + j*sizex + k*sizex*sizey]/(sizex*sizey*sizez));
				temp_phi3[i + j*sizex + k*sizex*sizey] = (pow(creal(phi_out[i + j*sizex + k*sizex*sizey]/(sizex*sizey*sizez)),2.0) +
				                                          pow(cimag(phi_out[i + j*sizex + k*sizex*sizey]/(sizex*sizey*sizez)),2.0));
			}
		}
	}
    ...
    for(k=kstart; k < kend; k++)
	{
		for(j=jstart; j < jend; j++)
		{
			for(i=istart; i < iend; i++)
			{
				...
				if (!isnan(temp_phi1[ index ])) {
				scalar[1][index] = temp_phi1[ index ];
				}
				if (!isnan(temp_phi2[ index ])) {
				scalar[2][index] = temp_phi2[ index ];
				}
				if (!isnan(temp_phi3[ index ])) {
                scalar[3][index] = temp_phi3[ index ];
                }
			}
		}
	}
