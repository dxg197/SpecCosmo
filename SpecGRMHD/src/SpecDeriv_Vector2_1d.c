...
void SpecDeriv_Write_Vector2_1d( CCTK_ARGUMENTS, CCTK_REAL ****vector2 );
void SpecDeriv_SpecGalMethod_Vector2_1d( CCTK_ARGUMENTS );
...
void SpecDeriv_Vector_Derivative2_1d( CCTK_ARGUMENTS, CCTK_REAL ****vector2 )
{
	...
	SpecDeriv_SpecGalMethod_Vector2_1d( CCTK_PASS_CTOC );
	SpecDeriv_Write_Vector2_1d( CCTK_PASS_CTOC, vector2 );
}
...
void SpecDeriv_SpecGalMethod_Vector2_1d( CCTK_ARGUMENTS )
{
    ...
    Derivative_SGM33x = fftw_alloc_complex(b_alloc_local);
    Derivative_SGM33y = fftw_alloc_complex(b_alloc_local);
    Derivative_SGM33z = fftw_alloc_complex(b_alloc_local);
    ...
    for(k = 0; k < b_local_n0; k++) 
    {
                ...
                Derivative_SGM33x[k] = -1.0 * Fourier_3D_kwaveN3 * Fourier_3D_kwaveN3 * phi_inx[k];
                Derivative_SGM33y[k] = -1.0 * Fourier_3D_kwaveN3 * Fourier_3D_kwaveN3 * phi_iny[k];
                Derivative_SGM33z[k] = -1.0 * Fourier_3D_kwaveN3 * Fourier_3D_kwaveN3 * phi_inz[k];
    }
    ...
    plan_backward33x = fftw_mpi_plan_dft_1d(sizez,Derivative_SGM33x,Derivative_SGM33x,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_PATIENT);
    plan_backward33y = fftw_mpi_plan_dft_1d(sizez,Derivative_SGM33y,Derivative_SGM33y,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_PATIENT);
    plan_backward33z = fftw_mpi_plan_dft_1d(sizez,Derivative_SGM33z,Derivative_SGM33z,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_PATIENT);
    ...
    for(k = 0; k < b_local_n1; k++) 
    {
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
				...
				temp_phi11x[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi22x[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi33x[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM33x[k]) / (sizez);
				temp_phi11y[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi22y[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi33y[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM33y[k]) / (sizez);
				temp_phi11z[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi22z[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi33z[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM33z[k]) / (sizez);
				temp_phi12x[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi13x[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi23x[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi12y[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi13y[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi23y[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi12z[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi13z[ i + j*sizex + k*sizex*sizey ] = 0.0;
				temp_phi23z[ i + j*sizex + k*sizex*sizey ] = 0.0;
			}
		}
	}
