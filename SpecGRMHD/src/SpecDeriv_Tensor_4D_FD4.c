...
void SpecDeriv_Tensor_Derivative_4D_FD4( CCTK_ARGUMENTS, CCTK_REAL ****tensor_4D );
        ...
        for (m = 1; m < 4; m++) {
            for (n = 0; n < 4; n++) {
                for (p = 0; p < 4; p++) {
                    ...
                    tensor_4D[m][n][p][index] = (tensor_4D[0][n][p][vv2index] - 8.0*tensor_4D[0][n][p][vvindex] + 
                                                8.0*tensor_4D[0][n][p][uuindex] - tensor_4D[0][n][p][uu2index]) / 
