...
void SpecDeriv_Write_Tensor( CCTK_ARGUMENTS, CCTK_REAL ****tensor );
void SpecDeriv_SpecGalMethod_Tensor( CCTK_ARGUMENTS );
		...
		for(j=0; j < jend; j++)
		{
			for(i=0; i < iend; i++)
			{
				...
				for(m=1; m < 4; m++) {
                	for(n=1; n < 4; n++) {
                		for(p=1; p < 4; p++) {
                			tensor[m][n][p][index] = 0.0;
                		}
                	}
                }
                
                temp_phi0xx[ index ] = tensor[0][1][1][index];
                temp_phi0yy[ index ] = tensor[0][2][2][index];
                temp_phi0zz[ index ] = tensor[0][3][3][index];
                temp_phi0xy[ index ] = tensor[0][1][2][index];
                temp_phi0xz[ index ] = tensor[0][1][3][index];
                temp_phi0yz[ index ] = tensor[0][2][3][index];
                temp_phi0yx[ index ] = tensor[0][2][1][index];
                temp_phi0zx[ index ] = tensor[0][3][1][index];
                temp_phi0zy[ index ] = tensor[0][3][2][index];
                
			}
		}
	}
	SpecDeriv_SpecGalMethod_Tensor( CCTK_PASS_CTOC );
	SpecDeriv_Write_Tensor( CCTK_PASS_CTOC, tensor );
}
...
void SpecDeriv_SpecGalMethod_Tensor( CCTK_ARGUMENTS )
{
    ...
    fftw_plan plan_forwardyx;
    fftw_plan plan_forwardzx;
    fftw_plan plan_forwardzy;
    ...
    fftw_plan plan_backward1yx;
    fftw_plan plan_backward2yx;
    fftw_plan plan_backward3yx;
    fftw_plan plan_backward1zx;
    fftw_plan plan_backward2zx;
    fftw_plan plan_backward3zx;
    fftw_plan plan_backward1zy;
    fftw_plan plan_backward2zy;
    fftw_plan plan_backward3zy;
    ...
    fftw_complex *Derivative_SGM1yx, *Derivative_SGM1zy, *Derivative_SGM1zx;
    fftw_complex *Derivative_SGM2yx, *Derivative_SGM2zy, *Derivative_SGM2zx;
    fftw_complex *Derivative_SGM3yx, *Derivative_SGM3zy, *Derivative_SGM3zx;
    fftw_complex *phi_inxx, *phi_inyy, *phi_inzz;
    fftw_complex *phi_inxy, *phi_inxz, *phi_inyz;
    fftw_complex *phi_inyx, *phi_inzx, *phi_inzy;
    ...
    fftw_complex *Derivative_SGM1yx_out, *Derivative_SGM1zy_out, *Derivative_SGM1zx_out;
    fftw_complex *Derivative_SGM2yx_out, *Derivative_SGM2zy_out, *Derivative_SGM2zx_out;
    fftw_complex *Derivative_SGM3yx_out, *Derivative_SGM3zy_out, *Derivative_SGM3zx_out;
    fftw_complex *phi_outxx, *phi_outyy, *phi_outzz;
    fftw_complex *phi_outxy, *phi_outxz, *phi_outyz;
    fftw_complex *phi_outyx, *phi_outzx, *phi_outzy;
    ...
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info1, -1,
       CCTK_VARIABLE_REAL, temp_phi0yx,
       CCTK_VARIABLE_REAL, temp_phi0yx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info1, -1,
       CCTK_VARIABLE_REAL, temp_phi0zx,
       CCTK_VARIABLE_REAL, temp_phi0zx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info1, -1,
       CCTK_VARIABLE_REAL, temp_phi0zy,
       CCTK_VARIABLE_REAL, temp_phi0zy);
    assert (! ierr);
    ...
    phi_inyx = fftw_alloc_complex(alloc_local);
    phi_inzx = fftw_alloc_complex(alloc_local);
    phi_inzy = fftw_alloc_complex(alloc_local);
    ...
    phi_outyx = fftw_alloc_complex(alloc_local);
    phi_outzx = fftw_alloc_complex(alloc_local);
    phi_outzy = fftw_alloc_complex(alloc_local);
    ...
    for(k = 0; k < local_n0; k++) 
    {
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
              ...
              phi_inyx[i + j*sizex + k*sizex*sizey] = (fftw_complex)temp_phi0yx[i + j*sizex + k*sizex*sizey];
              phi_inzx[i + j*sizex + k*sizex*sizey] = (fftw_complex)temp_phi0zx[i + j*sizex + k*sizex*sizey];
              phi_inzy[i + j*sizex + k*sizex*sizey] = (fftw_complex)temp_phi0zy[i + j*sizex + k*sizex*sizey];
              ...
              phi_outyx[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
              phi_outzx[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
              phi_outzy[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
            }
    ...
    plan_forwardyx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,phi_inyx,phi_outyx,MPI_COMM_WORLD,FFTW_FORWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_forwardzx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,phi_inzx,phi_outzx,MPI_COMM_WORLD,FFTW_FORWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_forwardzy = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,phi_inzy,phi_outzy,MPI_COMM_WORLD,FFTW_FORWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    ...
    fftw_execute(plan_forwardyx);
    fftw_execute(plan_forwardzx);
    fftw_execute(plan_forwardzy);
    ...
    Derivative_SGM1yx = fftw_alloc_complex(alloc_local);
    Derivative_SGM2yx = fftw_alloc_complex(alloc_local);
    Derivative_SGM3yx = fftw_alloc_complex(alloc_local);
    Derivative_SGM1zx = fftw_alloc_complex(alloc_local);
    Derivative_SGM2zx = fftw_alloc_complex(alloc_local);
    Derivative_SGM3zx = fftw_alloc_complex(alloc_local);
    Derivative_SGM1zy = fftw_alloc_complex(alloc_local);
    Derivative_SGM2zy = fftw_alloc_complex(alloc_local);
    Derivative_SGM3zy = fftw_alloc_complex(alloc_local);
    ...
    Derivative_SGM1yx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM2yx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM3yx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM1zx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM2zx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM3zx_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM1zy_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM2zy_out = fftw_alloc_complex(alloc_local);
    Derivative_SGM3zy_out = fftw_alloc_complex(alloc_local);
        ...
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
                ...
                Derivative_SGM3yz[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN3
                                            * phi_outyz[i + j*sizex + k*sizex*sizey];
                Derivative_SGM1yx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN1
                                            * phi_outxy[i + j*sizex + k*sizex*sizey]; 
                Derivative_SGM2yx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN2
                                            * phi_outxy[i + j*sizex + k*sizex*sizey];
                Derivative_SGM3yx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN3
                                            * phi_outxy[i + j*sizex + k*sizex*sizey];
                Derivative_SGM1zx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN1
                                            * phi_outxz[i + j*sizex + k*sizex*sizey]; 
                Derivative_SGM2zx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN2
                                            * phi_outxz[i + j*sizex + k*sizex*sizey];
                Derivative_SGM3zx[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN3
                                            * phi_outxz[i + j*sizex + k*sizex*sizey];
                Derivative_SGM1zy[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN1
                                            * phi_outyz[i + j*sizex + k*sizex*sizey]; 
                Derivative_SGM2zy[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN2
                                            * phi_outyz[i + j*sizex + k*sizex*sizey];
                Derivative_SGM3zy[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER 
                                            * Fourier_3D_kwaveN3
                                            * phi_outyz[i + j*sizex + k*sizex*sizey];
                ...
                Derivative_SGM1yx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO; 
                Derivative_SGM2yx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
                Derivative_SGM3yx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
                Derivative_SGM1zx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO; 
                Derivative_SGM2zx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
                Derivative_SGM3zx_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
                Derivative_SGM1zy_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO; 
                Derivative_SGM2zy_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
                Derivative_SGM3zy_out[i + j*sizex + k*sizex*sizey] = ZERO_COMPLEXNO;
    ...
    plan_backward1yx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM1xy,Derivative_SGM1xy_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward2yx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM2xy,Derivative_SGM2xy_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward3yx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM3xy,Derivative_SGM3xy_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward1zx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM1xz,Derivative_SGM1xz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward2zx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM2xz,Derivative_SGM2xz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward3zx = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM3xz,Derivative_SGM3xz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward1zy = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM1yz,Derivative_SGM1yz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward2zy = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM2yz,Derivative_SGM2yz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    plan_backward3zy = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM3yz,Derivative_SGM3yz_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_WISDOM_ONLY | FFTW_ESTIMATE);
    ...
    fftw_execute(plan_backward1yx);
    fftw_execute(plan_backward2yx);
    fftw_execute(plan_backward3yx);
    fftw_execute(plan_backward1zx);
    fftw_execute(plan_backward2zx);
    fftw_execute(plan_backward3zx);
    fftw_execute(plan_backward1zy);
    fftw_execute(plan_backward2zy);
    fftw_execute(plan_backward3zy);
    ...
    for(k = 0; k < local_n0; k++) 
    {
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
				...
				temp_phi1yx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM1yx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi2yx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM2yx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi3yx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM3yx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi1zx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM1zx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi2zx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM2zx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi3zx[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM3zx_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi1zy[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM1zy_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi2zy[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM2zy_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
				temp_phi3zy[ i + j*sizex + k*sizex*sizey ] = creal(Derivative_SGM3zy_out[i + j*sizex + k*sizex*sizey]) / (sizex*sizey*sizez);
			}
		}
	}
    ...
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi1yx,
       CCTK_VARIABLE_REAL, temp_phi1yx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi2yx,
       CCTK_VARIABLE_REAL, temp_phi2yx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi3yx,
       CCTK_VARIABLE_REAL, temp_phi3yx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi1zx,
       CCTK_VARIABLE_REAL, temp_phi1zx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi2zx,
       CCTK_VARIABLE_REAL, temp_phi2zx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi3zx,
       CCTK_VARIABLE_REAL, temp_phi3zx);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi1zy,
       CCTK_VARIABLE_REAL, temp_phi1zy);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi2zy,
       CCTK_VARIABLE_REAL, temp_phi2zy);
    assert (! ierr);
    
    ierr = Slab_Transfer
      (cctkGH, cctk_dim, info2, -1,
       CCTK_VARIABLE_REAL, temp_phi3zy,
       CCTK_VARIABLE_REAL, temp_phi3zy);
    assert (! ierr);
    ...
    fftw_destroy_plan(plan_forwardyx);
    fftw_destroy_plan(plan_forwardzx);
    fftw_destroy_plan(plan_forwardzy);
    ...
    fftw_destroy_plan(plan_backward1yx);
    fftw_destroy_plan(plan_backward2yx);
    fftw_destroy_plan(plan_backward3yx);
    fftw_destroy_plan(plan_backward1zx);
    fftw_destroy_plan(plan_backward2zx);
    fftw_destroy_plan(plan_backward3zx);
    fftw_destroy_plan(plan_backward1zy);
    fftw_destroy_plan(plan_backward2zy);
    fftw_destroy_plan(plan_backward3zy); 
    fftw_free(phi_inxx);
    fftw_free(phi_inyy);
    fftw_free(phi_inzz);
    fftw_free(phi_inxy);
    ...
    fftw_free(Derivative_SGM1yx);
    fftw_free(Derivative_SGM2yx);
    fftw_free(Derivative_SGM3yx);
    fftw_free(Derivative_SGM1zx);
    fftw_free(Derivative_SGM2zx);
    fftw_free(Derivative_SGM3zx);
    fftw_free(Derivative_SGM1zy);
    fftw_free(Derivative_SGM2zy);
    fftw_free(Derivative_SGM3zy);
    fftw_free(phi_outxx);
    fftw_free(phi_outyy);
    fftw_free(phi_outzz);
    fftw_free(phi_outxy);
    ...
    fftw_free(Derivative_SGM1yx_out);
    fftw_free(Derivative_SGM2yx_out);
    fftw_free(Derivative_SGM3yx_out);
    fftw_free(Derivative_SGM1zx_out);
    fftw_free(Derivative_SGM2zx_out);
    fftw_free(Derivative_SGM3zx_out);
    fftw_free(Derivative_SGM1zy_out);
    fftw_free(Derivative_SGM2zy_out);
    fftw_free(Derivative_SGM3zy_out);
    fftw_mpi_cleanup();
    fftw_cleanup();
}
...
void SpecDeriv_Write_Tensor( CCTK_ARGUMENTS, CCTK_REAL ****tensor )
{
    ...
    for(k=0; k < kstart; k++)
	{
		for(j=0; j < jstart; j++)
		{
            ...
            {
            	index = CCTK_GFINDEX3D( cctkGH, i, j, k );
                tensor[1][1][1][index] = 0.0;
                tensor[2][1][1][index] = 0.0;
                tensor[3][1][1][index] = 0.0;
                tensor[1][2][2][index] = 0.0;
                tensor[2][2][2][index] = 0.0;
                tensor[3][2][2][index] = 0.0;
                tensor[1][3][3][index] = 0.0;
                tensor[2][3][3][index] = 0.0;
                tensor[3][3][3][index] = 0.0;
                
                tensor[1][1][2][index] = 0.0;
                tensor[2][1][2][index] = 0.0;
                tensor[3][1][2][index] = 0.0;
                tensor[1][1][3][index] = 0.0;
                tensor[2][1][3][index] = 0.0;
                tensor[3][1][3][index] = 0.0;
                tensor[1][2][3][index] = 0.0;
                tensor[2][2][3][index] = 0.0;
                tensor[3][2][3][index] = 0.0;
                
                tensor[1][2][1][index] = 0.0;
                tensor[2][2][1][index] = 0.0;
                tensor[3][2][1][index] = 0.0;
                tensor[1][3][1][index] = 0.0;
                tensor[2][3][1][index] = 0.0;
                tensor[3][3][1][index] = 0.0;
                tensor[1][3][2][index] = 0.0;
                tensor[2][3][2][index] = 0.0;
                tensor[3][3][2][index] = 0.0;
            }
        }
    }
    ...
    for(k=kstart; k < kend; k++)
	{
		for(j=jstart; j < jend; j++)
		{
            ...
            {
				index = CCTK_GFINDEX3D( cctkGH, i, j, k );
                tensor[1][1][1][index] = temp_phi1xx[ index ];
                tensor[2][1][1][index] = temp_phi2xx[ index ];
                tensor[3][1][1][index] = temp_phi3xx[ index ];
                tensor[1][2][2][index] = temp_phi1yy[ index ];
                tensor[2][2][2][index] = temp_phi2yy[ index ];
                tensor[3][2][2][index] = temp_phi3yy[ index ];
                tensor[1][3][3][index] = temp_phi1zz[ index ];
                tensor[2][3][3][index] = temp_phi2zz[ index ];
                tensor[3][3][3][index] = temp_phi3zz[ index ];
                
                tensor[1][1][2][index] = temp_phi1xy[ index ];
                tensor[2][1][2][index] = temp_phi2xy[ index ];
                tensor[3][1][2][index] = temp_phi3xy[ index ];
                tensor[1][1][3][index] = temp_phi1xz[ index ];
                tensor[2][1][3][index] = temp_phi2xz[ index ];
                tensor[3][1][3][index] = temp_phi3xz[ index ];
                tensor[1][2][3][index] = temp_phi1yz[ index ];
                tensor[2][2][3][index] = temp_phi2yz[ index ];
                tensor[3][2][3][index] = temp_phi3yz[ index ];
                
                tensor[1][2][1][index] = temp_phi1yx[ index ];
                tensor[2][2][1][index] = temp_phi2yx[ index ];
                tensor[3][2][1][index] = temp_phi3yx[ index ];
                tensor[1][3][1][index] = temp_phi1zx[ index ];
                tensor[2][3][1][index] = temp_phi2zx[ index ];
                tensor[3][3][1][index] = temp_phi3zx[ index ];
                tensor[1][3][2][index] = temp_phi1zy[ index ];
                tensor[2][3][2][index] = temp_phi2zy[ index ];
                tensor[3][3][2][index] = temp_phi3zy[ index ];
			}
		}
	}
}
