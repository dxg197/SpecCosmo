...
void SpecDeriv_Write_Vector( CCTK_ARGUMENTS, CCTK_REAL ***vector );
void SpecDeriv_SpecGalMethod_Vector( CCTK_ARGUMENTS );
...
void SpecDeriv_Vector_Derivative( CCTK_ARGUMENTS, CCTK_REAL ***vector )
{
    ...
    CCTK_INT i, j, k, m, n, iend, jend, kend, index;
    ...
    for(k=0; k < kend; k++)
	{
		for(j=0; j < jend; j++)
		{
			for(i=0; i < iend; i++)
			{
                ...
                for(m=1; m < 4; m++) {
                	for(n=1; n < 4; n++) {
                		vector[m][n][index] = 0.0;
                	}
                }
			}
		}
	}
	SpecDeriv_SpecGalMethod_Vector( CCTK_PASS_CTOC );
	SpecDeriv_Write_Vector( CCTK_PASS_CTOC, vector );
}
