...
contains
  !
  subroutine makecompound(CCTK_ARGUMENTS,i,j,k)
    ...
    if ((z(i,j,k).ge.-0.025).and.(z(i,j,k).le.0.025)) then
    
    ! Calc variables in geometerized units
    
    uu4(3) = maxvel*((1.85) + z(i,j,k)*74.0)
    uu4(2) = maxvel*((2.88) + z(i,j,k)*115.2)
    uu4(1) = 0.0
    ...
    bb3(3) = 3.0*SIE
    bb3(2) = (-1.9285 + z(i,j,k)*197.14)*SIE
    bb3(1) = 0.0
    rho0   = 1.0*SIE
    pp0 = 1.0*SIE
