...
module periodic_shock
  ...
  implicit none
  private
  public makeperiodicshock
  !
contains
  !
  subroutine makeperiodicshock(CCTK_ARGUMENTS,i,j,k)
    ...
    if (abs(z(i,j,k)).gt.25.0) then
    
    ! Calc variables in geometerized units
    
    uu4(3) = maxvel*(outer_uz) 
    uu4(2) = maxvel*(outer_uy)
    uu4(1) = maxvel*(outer_ux)
    ...
    if (abs(z(i,j,k)).le.25.0) then
    
    ! Calc variables in geometerized units
    
    uu4(3) = maxvel*(inner_uz) 
    uu4(2) = maxvel*(inner_uy)
    uu4(1) = maxvel*(inner_ux)
	...
	do m = 1,3	
  	 beta(m) = (bb3(m)/alpha(i,j,k) + beta(0)*uu4(m))/(sqrt(4.0*pi)*uu4(0))
	end do
  ...
  end subroutine makeperiodicshock
  !
end module periodic_shock
