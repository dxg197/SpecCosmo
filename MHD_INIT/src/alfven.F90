...
contains
  !
  subroutine makealfven(CCTK_ARGUMENTS,i,j,k)
    ...
    if ((z(i,j,k).ge.-0.25).and.(z(i,j,k).le.0.25)) then
    
    ! Calc variables in geometerized units
    
    uu4(3) = maxvel*((1.85) + z(i,j,k)*7.4)
    uu4(2) = maxvel*((2.88) + z(i,j,k)*11.52)
    uu4(1) = 0.0
    ...
    bb3(3) = 3.0*SIE
    bb3(2) = (-1.9285 + z(i,j,k)*19.714)*SIE
    bb3(1) = 0.0
    rho0   = 1.0*SIE
    pp0 = 1.0*SIE
