...
contains
  !
  subroutine maketurbmhd(CCTK_ARGUMENTS, volume, radius_max, i,j,k)
    DECLARE_CCTK_ARGUMENTS
    DECLARE_CCTK_PARAMETERS 
    !    
    INTEGER, intent(in) :: i,j,k
    INTEGER :: shape(3), pos, m, n
    CCTK_REAL, intent(in) :: radius_max, volume
    CCTK_REAL :: rhostar0, rhodstar0, tau0, s0(3), sd0(3), bb0(3), vv0(3), Tmunu(0:3,0:3)
    CCTK_REAL :: sqrtg, detg, g(3,3), g_inv(0:3,0:3), gg(0:3,0:3), zero, rand
    CCTK_REAL :: uu4(0:3), bb3(3), rho0, rho_d0, eps, pp0, tau_dark0, Tdark(0:3,0:3)
    CCTK_REAL :: shift(3), hp, beta(0:3), uu2, kappa, rho_c, one
    
    CCTK_REAL :: angvel(3), radius, rphix, rphiy, rphiz
    CCTK_REAL :: xvelocityx, yvelocityx, zvelocityx, velocityx
    CCTK_REAL :: xvelocityy, yvelocityy, zvelocityy, velocityy
    CCTK_REAL :: xvelocityz, yvelocityz, zvelocityz, velocityz
    CCTK_REAL :: xx(3), rthetax, rthetay, rthetaz
    ...
    radius = SQRT(x(i,j,k)*x(i,j,k)+y(i,j,k)*y(i,j,k)+z(i,j,k)*z(i,j,k))
    
    if (radius < radius_max) then
    velocityx = angvel(1)*radius/radius_max
    velocityy = angvel(2)*radius/radius_max
    velocityz = angvel(3)*radius/radius_max
    else
    velocityx = 0.0
    velocityy = 0.0
    velocityz = 0.0
    ...
    if (radius > 0.0) then
    rthetax = acos(x(i,j,k)/radius)
    rphix = atan2(z(i,j,k),y(i,j,k))
    rthetay = acos(y(i,j,k)/radius)
    rphiy = atan2(x(i,j,k),z(i,j,k))
    rthetaz = acos(z(i,j,k)/radius)
    rphiz = atan2(y(i,j,k),x(i,j,k))
    end if
    
    xvelocityx = 0
    yvelocityx = -velocityx*sin(rthetax)*sin(rphix)
    zvelocityx = velocityx*sin(rthetax)*cos(rphix)
    xvelocityy = velocityy*sin(rthetay)*cos(rphiy)
    yvelocityy = 0
    zvelocityy = -velocityy*sin(rthetay)*sin(rphiy)
    xvelocityz = -velocityz*sin(rthetaz)*sin(rphiz)
    yvelocityz = velocityz*sin(rthetaz)*cos(rphiz)
    zvelocityz = 0
    ...
    if (random.ne.zero) then
    call RANDOM_NUMBER(rand)
    else 
    rand = 1.0
    endif
    uu4(1) = 2.0*maxvel*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + xvelocityx + xvelocityy + xvelocityz
    ...
    uu4(2) = 2.0*maxvel*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + yvelocityx + yvelocityy + yvelocityz
    ...
    uu4(3) = 2.0*maxvel*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + zvelocityx + zvelocityy + zvelocityz
    ...
    bb3(1) = 2.0*maxbb*Gauss*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + bb3(1)
    ...
    bb3(2) = 2.0*maxbb*Gauss*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + bb3(2)
    ...
    bb3(3) = 2.0*maxbb*Gauss*(rand - 0.5)*EXP(-0.5*(stdv*radius)**2) + bb3(3)
    ...
    if (irho.ne.zero) then
       eps = (eps_cof*Rad_Const*SIE*(itemp**4) - beta2(i,j,k)*(1.0-0.5/alpha(i,j,k)**2)/Density - irho_dark)/irho - 1.0
    else
       eps = 0.0
    end if
    
    if ((nonrel.ne.zero).and.((irho).ne.zero)) then
