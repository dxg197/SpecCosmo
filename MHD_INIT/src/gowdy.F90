! MHD_Init
! Gowdy: Analytic Data for Gowdy Cosmology:
...
contains
  !
  subroutine makegowdy (time, z, g, kk, lz)
    use cactus
    ...
    !
    CCTK_REAL, intent(in) :: time, lz, z
    CCTK_REAL, intent(out) :: g(3,3), kk(3,3) 
    CCTK_REAL  th0,gc,pi,ppdot,lamdot,t,lam,pp,bj0,bj1,by0,by1
    integer gowdy_type
    integer i
    !
    ...
    select case (gowdy_type)
     ...
     case (gowdy_collapse)
      ...
      pp = Amp*bj0*COS(z)
      ppdot = -Amp*bj1*COS(z)  
      lam = Amp**2*(-t*bj0*bj1*COS(z)**2 + 0.5*(t*bj0)**2 + &
            0.5*(t*bj1)**2) 

      lamdot = Amp**2*(t*COS(z)**2*bj1**2+t*SIN(z)**2*bj0**2) 
  
    g(3,3) = t**(-0.5)*EXP(0.5*lam) 
     
    g(2,2) = t*EXP(pp) 
    
    g(1,1) = t*EXP(-pp)
    
    kk(3,3) = -(lamdot*EXP(0.5*lam)/(2.0*t**(0.5))-EXP(0.5*lam)/(2.0*t**(1.5)))*t**(0.25)/(2.0*EXP(0.25*lam))  
    
    kk(2,2) = -(ppdot*EXP(pp)*t+EXP(pp))*t**(0.25)/(2.0*EXP(0.25*lam)) 
    
    kk(1,1) = -(EXP(-pp)-t*ppdot*EXP(-pp))*t**(0.25)/(2.0*EXP(0.25*lam)) 
    !
  end subroutine makegowdy
  !
