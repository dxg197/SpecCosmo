...
void FFT_INIT_Derivative( CCTK_ARGUMENTS);
void FFT_INIT_START( CCTK_ARGUMENTS );
...
void FFT_INIT_START( CCTK_ARGUMENTS)
{
    ...
    CCTK_INT i, j, k, iend, jend, kend, index, ierr, handle;
    CCTK_REAL lx, ly, lz, xmin, xmax, ymin, ymax, zmin, zmax;
    CCTK_REAL Kgrid_res = 8.0*atan(1.0), nw = 2.0;
    ...
    for(k=0; k < cctk_lsh[2]; k++)
	{
		for(j=0; j < cctk_lsh[1]; j++)
		{
			for(i=0; i < cctk_lsh[0]; i++)
			{
                ...
                temp_phi0[ index ] = -(nw*Kgrid_res/lx) * sin( nw*Kgrid_res * x[index]/lx)*cos( nw*Kgrid_res * y[index]/ly)*cos( nw*Kgrid_res * z[index]/lz);
			}
		}
	}
	
	FFT_INIT_Derivative( CCTK_PASS_CTOC );
}
...
void FFT_INIT_Derivative( CCTK_ARGUMENTS )
{
    ...
    CCTK_REAL lx, ly, lz, xmin, xmax, ymin, ymax, zmin, zmax;
    fftw_complex *phi_in, *phi_out; 
    fftw_complex *Derivative_SGM1, *Derivative_SGM1_out;
    fftw_complex *Derivative_SGM2, *Derivative_SGM2_out;
    fftw_complex *Derivative_SGM3, *Derivative_SGM3_out; 
    ...
    plan_forward = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,phi_in,phi_out,MPI_COMM_WORLD,FFTW_FORWARD,FFTW_EXHAUSTIVE);
    fftw_execute(plan_forward);
    
    //  Multiply i*k factor to FFT, create Derivative_SGM
        ...
        for(j = 0; j < sizey; j++) 
        {
            for(i = 0; i < sizex; i++) 
            {
                ...
                Derivative_SGM1[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER * 
                                   Fourier_3D_kwaveN1 * phi_out[i + j*sizex + k*sizex*sizey]; 
                
                Derivative_SGM2[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER * 
                                   Fourier_3D_kwaveN2 * phi_out[i + j*sizex + k*sizex*sizey];
                
                Derivative_SGM3[i + j*sizex + k*sizex*sizey] = IMAG_NUMBER * 
                                   Fourier_3D_kwaveN3 * phi_out[i + j*sizex + k*sizex*sizey];
    ...
    plan_backward1 = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM1,Derivative_SGM1_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_EXHAUSTIVE);
    plan_backward2 = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM2,Derivative_SGM2_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_EXHAUSTIVE);
    plan_backward3 = fftw_mpi_plan_dft_3d(sizex,sizey,sizez,Derivative_SGM3,Derivative_SGM3_out,MPI_COMM_WORLD,FFTW_BACKWARD,FFTW_EXHAUSTIVE);
    ...
    if (rank == 0) fftw_export_wisdom_to_filename("fftw.wis");
